import { isDemoMode, supabaseAdmin as supabase } from './supabase.js';
...
function scoreMatch(itemText, queryTokens) {
    if (queryTokens.length === 0) return 0;
    const norm = normalizeText(itemText);
    return queryTokens.reduce((s, t) => s + (norm.includes(t) ? 1 : 0), 0);
}

export function overlapRatio(itemText, queryTokens) {
    if (queryTokens.length === 0) return 0;
    const norm = normalizeText(itemText);
    return queryTokens.filter(t => norm.includes(t)).length / queryTokens.length;
}
...
//  USER
// ────────────────────────────────────────────
export async function createUser(displayName = 'Friend', timezone = null, telegramId = null) {
    const user = {
        id: uuidv4(),
    ...
    const insertData = { display_name: displayName, timezone };
...
export async function saveMemoryItem(userId, content, category = 'Note', remindAt = null) {
    ...
    if (isDemoMode) {
        ...
        for (const e of existing) {
            if (overlapRatio(e.content, newTokens) >= 0.8) return e;
        }
...
export async function deleteMemoryItemByContent(userId, searchText) {
    ...
    const findBest = (items) => {
        ...
        for (const item of items) {
            const score = scoreMatch(item.content, queryTokens);
    ...
    const best = findBest(candidates);
    if (!best) return null;
...
export async function archiveAllDuplicates(userId, contentHint) {
    ...
    if (isDemoMode) {
        const items = (demoStore.memoryItems.get(userId) || [])
            .filter(i => i.status === 'Active')
            .sort((a, b) => new Date(b.captured_at) - new Date(a.captured_at));
        const matches = items.filter(i => scoreMatch(i.content, queryTokens) >= 1);
    ...
    const matches = candidates.filter(i => scoreMatch(i.content, queryTokens) >= 1);
...
export async function getPreviousSession(userId) {
    if (isDemoMode) {
        const today = todayKey(userId);
        let latest = null;
        for (const [key, session] of demoStore.sessions) {
            if (session.user_id === userId && key !== today) {
                if (!latest || session.started_at > latest.started_at) latest = session;
            }
        }
        return latest;
    }
...
/**
 * Get all sessions with due check-ins, joined with users that have a telegram_id.
 */
export async function getAllSessionsWithDueCheckIns() {
    const now = new Date().toISOString();
    if (isDemoMode) {
        const results = [];
        for (const [, session] of demoStore.sessions) {
            if (session.check_in_due_at && session.check_in_due_at <= now) {
                const user = demoStore.users.get(session.user_id);
                if (user?.telegram_id) {
    ...
    const { data, error } = await supabase
        .from('sessions')
        .select('*, users!inner(id, display_name, telegram_id, timezone, main_focus, biggest_struggle)')
        .not('check_in_due_at', 'is', null)
        .lte('check_in_due_at', now)
        .not('users.telegram_id', 'is', null);
    if (error) throw error;
    return (data || []).map(row => ({
        session: { id: row.id, user_id: row.user_id, check_in_due_at: row.check_in_due_at, briefing_delivered: row.briefing_delivered, active_task_id: row.active_task_id },
        user: row.users,
    }));
}
