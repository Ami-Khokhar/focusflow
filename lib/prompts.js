...
7. **General conversation**: For anything else:
...
export async function classifyIntentWithLLM(message, apiKey, history = []) {
    try {
        ...
        // Give the model a one-line summary of the last assistant message for check-in context
        const lastAssistant = [...history].reverse().find((m) => m.role === 'assistant');
        const context = lastAssistant
            ? `Last assistant message: "${lastAssistant.content.slice(0, 120)}"`
            : '';
