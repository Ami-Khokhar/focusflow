 ...
 */
export function buildSystemPrompt({ userName, currentTime, timezone, mode, memoryItems = [], activeCheckIn = false, checkInDueAt = null, remindAt = null, mainFocus = null, biggestStruggle = null }) {
...
8. **General conversation**: For anything else:
...
/**
 * LLM-based intent classifier — PRIMARY classifier when Groq is available.
 * Regex is the fallback (demo mode / API failure).
 * Uses llama-3.1-8b-instant for low latency (~200ms).
 *
...
export async function classifyIntentWithLLM(message, apiKey, history = []) {
    try {
        ...
        // Give the model the last assistant message for check-in context (longer slice for accuracy)
        ...
        const res = await client.chat.completions.create({
            ...
            messages: [{
                role: 'user',
                content: `Classify the user message into ONE category. Return ONLY valid JSON, nothing else.
...
Return JSON: {"intent": "...", "confidence": 0.0}`,
        ...
        // Lower threshold for check_in_acceptance when agent was offering a check-in
