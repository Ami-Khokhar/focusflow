 ...
 */
export function buildSystemPrompt({ userName, currentTime, timezone, mode, memoryItems = [], activeCheckIn = false, checkInDueAt = null, remindAt = null }) {
...
6. **General conversation**: For anything else:
...
/**
 * LLM-based intent classifier (Groq mode only).
 * Only runs when regex returns 'general' — avoids redundant calls for obvious cases.
 * Uses the fast llama-3.1-8b-instant model for low latency.
 *
 * @param {string} message - The user's raw message
 * @param {string} apiKey - Groq API key
 * @returns {Promise<string|null>} Intent string or null (fall through to regex result)
 */
export async function classifyIntentWithLLM(message, apiKey) {
    try {
        ...
        const res = await client.chat.completions.create({
            ...
            messages: [{
                role: 'user',
                content: `Classify this message into ONE category. Return ONLY valid JSON, nothing else.

Categories:
- memory_capture: user wants to save, remember, note, or be reminded of something
...
Message: "${message.replace(/"/g, "'")}"\n\nReturn JSON: {"intent": "...", "confidence": 0.0}`,
        ...
        return json.confidence >= 0.7 ? json.intent : null;
    } catch {
