...
- Confirmations must be warm, not robotic. Say "Got it — I'll remind you at 4 PM" instead of "Reminder set: X at Y."
- NEVER start confirmations with "Reminder set:", "Reminder updated:", "I recall:", or similar robotic labels.
- For quick actions (save, delete, update), use short warm confirmations. Do NOT suggest focus sessions for simple confirmations.
- If the user indicates completion (done, finished, all done, completed), respond with encouragement like "Nice work finishing that!"
- When listing saved items, present them in friendly tone and optionally offer one tiny next step.
- Use natural conversational phrasing in all confirmations.
...
1. **Task stuck / decomposition request**: If the user says they're stuck on something or describes a large task:
   - Provide a single, concrete, immediately-actionable first step (not "write the report" but "open a blank document and type one sentence about what the report is about")
   - Do NOT ask a clarifying question first — jump straight to giving them a tiny first step.
   - You MUST end your response by asking exactly: "Want me to check in with you in 25 minutes?"
...
FEW-SHOT EXAMPLES (follow this tone and style):

User: "remind me to call mom in 10 minutes"
Assistant: "Got it — I'll remind you to call mom in 10 minutes."

User: "actually push that back 30 minutes"
Assistant: "No problem — I pushed that reminder back 30 minutes."

User: "done!"
Assistant: "Nice work finishing that! 🎉 What's next?"

User: "forget that"
Assistant: "Got it — I removed that."

User: "what do you remember?"
Assistant: "Here's what I have saved for you:\n\n• Call dentist\n• Finish project report\n\nWould you like help choosing one to start?"`;
...
- Do NOT ask a follow-up question.`;
        ...
        case 'memory_recall': {
            const itemsList = memoryItems.length > 0
                ? memoryItems.map((item) => `- [${item.category}] ${item.content}`).join('\n')
                : '(none)';
            modePrompt = `
TASK: The user asked what you remember.
- You MUST include every active item exactly once. No omissions.
- Items to include:\n${itemsList}
- If none, say you have nothing saved yet.
- Present the items warmly, e.g. "Here's what I have saved for you:"
- Use a bullet list for the items.
- Keep tone warm and concise.
- End with: "Would you like help choosing one to start?"
- Example response: "Here's what I have saved for you:\n\n• Call dentist\n• Finish project report\n\nWould you like help choosing one to start?"`;
            break;
        }

        case 'memory_delete':
            modePrompt = `
TASK: Confirm the last saved item was removed.
- You MUST respond with EXACTLY ONE short sentence confirming the deletion.
- Say "Got it — I removed that." or a slight variation.
- Do NOT list remaining items. Do NOT show what's left. Do NOT show a bullet list.
- Do NOT output an empty response. You MUST produce at least one sentence.
- Be warm and matter-of-fact. Under 14 words.
- Example output: "Got it — I removed that."`;
            break;
...
- Start with "Got it" or similar warm phrasing.
...
- Use a warm tone in ONE sentence: acknowledge + reminder content + time
- Keep it under 22 words
- Do NOT ask follow-up questions
- NEVER use "Reminder set:" as a label. Use natural phrasing.
- Example: "Got it — I'll remind you to ${userMessage} at ${timeStr}."`;
            break;
        }
        ...
        case 'reminder_reschedule': {
            ...
            modePrompt = `
TASK: Confirm you've rescheduled their reminder.
- Start with "Got it" or "No problem" — use warm, natural phrasing.
- The reminder has been rescheduled for exactly **${timeStr}** - use this exact time, do NOT recalculate
- Use the word "pushed" to convey rescheduling (e.g. "I pushed that reminder to...").
- Mention what you'll remind them about
- Be warm and concise in one sentence
- Keep it under 24 words
- NEVER use "Reminder updated:" as a label. Use natural phrasing.
- Do NOT ask follow-up questions
- Example: "No problem — I pushed that reminder to ${timeStr}."`;
            break;
        }

        case 'task_complete':
            modePrompt = `The user just completed a task! Celebrate this win enthusiastically but briefly.
    IMPORTANT: The user has FINISHED the task. Do NOT say they "started" it — they COMPLETED it.
    ...
    mention it by name.
    Example: "Nice work finishing that! 🎉 What's next?"`;
            break;
...
export function detectIntent(message) {
    ...
    if (/remind me|remember (this|that|these|to|the following)|i (need|want) to remember|note (this|that|these|down)|save (this|that|these)|don't let me forget|keep (a\s+)?(note|track) of/i.test(lower)) {
    ...
    // Forget/delete
    if (/forget (that|the\b|it)|delete (that|the\b|it)|remove (that|the\b|it)|undo that/i.test(lower)) {
    ...
    // Task completion
    if (/\b(all done|that'?s done|got it done|wrapped (it\s+)?up|done|finished|completed|checked off|did it|nailed it|knocked.*(out|off)|i finished it|i got it done)\b/i.test(lower)) {
...
export function detectCheckInAcceptance(conversationHistory, userMessage) {
    ...
    const affirmatives = /^(yes|yeah|yep|yup|sure|ok|okay|please|do it|sounds good|go ahead|let's do it|absolutely|definitely|that'd be great|ye|ya|yea)\b/i;
 ...
 */
export async function classifyIntentWithLLM(message, apiKeyOrHistory = [], maybeHistory = []) {
    const { getGroqClient } = await import('./groqClient.js');
    try {
        const apiKey = typeof apiKeyOrHistory === 'string' ? apiKeyOrHistory : '';
        const history = Array.isArray(apiKeyOrHistory)
            ? apiKeyOrHistory
            : (Array.isArray(maybeHistory) ? maybeHistory : []);
