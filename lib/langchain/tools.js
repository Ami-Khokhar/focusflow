 ...
 */
export function createTools(userId, sessionId, timezone, user, store = null) {
    const saveMemory = tool(
        async ({ content, category, remind_at, minutes_from_now }) => {
            ...
            // Dual-write to Store for cross-session retrieval
            if (store && item?.id) {
                store.put([userId, "items"], `item_${item.id}`, {
                    content, category: category || 'Note',
                    remind_at: finalRemindAt, captured_at: new Date().toISOString(),
                }).catch(e => console.warn("[Store] dual-write failed:", e.message));
            }
    ...
    const deleteMemory = tool(
        async ({ content_hint }) => {
            if (content_hint && content_hint.length >= 4) {
                ...
                // Dual-delete from Store
                if (store && count > 0) {
                    store.search([userId, "items"], { query: content_hint, limit: 3 })
                        .then(items => Promise.all(items.map(i => store.delete([userId, "items"], i.key))))
                        .catch(e => console.warn("[Store] dual-delete failed:", e.message));
                }
