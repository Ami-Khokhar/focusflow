...
export async function streamAgentResponse({
    ...
    // Regex to detect a trailing partial <function...> tag split across SSE chunks
    const PARTIAL_TAG_RE = /<(?:f(?:u(?:n(?:c(?:t(?:i(?:o(?:n(?:=)?)?)?)?)?)?)?)?)?$/;
    ...
    while (iterations < maxIterations) {
        ...
        try {
            ...
            for await (const chunk of stream) {
                // Emit text content tokens — guard against empty strings
                const token = chunk.content;
                if (token && typeof token === 'string') {
                    // Strip tool call XML that Llama models embed in content stream
                    const cleaned = (pendingBuffer + token)
                        .replace(/<function=\w+>[\s\S]*?<\/function>/g, '')
                        .replace(/<function=\w+>[\s\S]*/g, '')
                        .replace(/<\|python_tag\|>/g, '');
                    pendingBuffer = '';

                    // Check for a trailing partial tag that may complete in the next chunk
                    const partialMatch = cleaned.match(PARTIAL_TAG_RE);
                    if (partialMatch) {
                        const safePrefix = cleaned.slice(0, partialMatch.index);
                        pendingBuffer = cleaned.slice(partialMatch.index);
                        if (safePrefix.trim()) {
                            fullText += safePrefix;
                            iterationText += safePrefix;
                            onToken(safePrefix);
                        }
            ...
            // Flush any remaining pending buffer (apply full XML strip)
            if (pendingBuffer) {
            ...
            // Concatenate chunks to get full response with tool_calls
            response = chunks.reduce((acc, chunk) => {
                if (!acc) return chunk;
