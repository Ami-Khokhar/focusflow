...
// Check if any keys are available to determine demo mode
const hasAnyKey = process.env.GROQ_API_KEY || Object.keys(process.env).some(k => k.startsWith('GROQ_API_KEY_') && process.env[k]);
const isDemoMode = !hasAnyKey;
    ...
    while (attempts < maxAttempts) {
        try {
            ...
            const stream = await client.chat.completions.create({
                model: 'llama-3.1-8b-instant', // Downgraded from llama-3.3-70b-versatile to avoid TPD limits during testing
                messages,
                max_tokens: 500,
                temperature: 0.7,
                stream: true,
            ...
            attempts++;
            console.error(`[Attempt ${attempts}] Groq API error:`, error?.message, error?.status);
            ...
            // Wait 500ms before retrying with a likely different key
